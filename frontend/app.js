...
function showPage(name) { const titles = { transactions: "Movimientos", budgets: "Presupuestos" }; const content = name === "transactions" ? transactionPage() : budgetPage(); document.querySelector(".content").innerHTML = `<header class="topbar"><div><p class="eyebrow">LEDGER / ${titles[name].toUpperCase()}</p><h1>${titles[name]}</h1></div><button class="avatar" id="logout">↗</button></header><section class="surface section-page">${content}</section>`; nav(); }
